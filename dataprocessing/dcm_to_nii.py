...
def pad_arr(src_arr, ref_affine, img_dim: int, save_path: str, file_name: str):
    ...
    print(f'src_img_data.shape = {np.shape(src_img_data)}')
    # new_arr = np.zeros((np.shape(src_img_data)[0], np.shape(src_img_data)[1], img_dim))
    # create array filled with certain value
    new_arr = np.full((np.shape(src_img_data)[0], np.shape(src_img_data)[1], img_dim), -1024)
    print(f'new_arr.shape = {np.shape(new_arr)}')
    print(f'affine = {affine}')
...
def resample_nb(src_file: str, dst_file: str, ref_file: str, save_path: str):
    ...
    # print(f'src_img.affine = {src_img.affine}, ref_img.affine = {ref_img.affine}')
    src_img_data = src_img.get_fdata()
...
for f in file_path:
    ...
    ct_arr = resample_nb(src_file=ct_nii[0], dst_file='CT_rsmpl.nii.gz', ref_file=f + 'MASK_rsmpl.nii.gz', save_path=f)
    spect_arr = resample_nb(src_file=spect_nii[0], dst_file='SPECT_rsmpl.nii.gz', ref_file=f + 'MASK_rsmpl.nii.gz', save_path=f)
