...
def crop_file(folder_path):
    ...
    sitk.WriteImage(cropped_ct_img[:, :, :], 'crop_ct.nii.gz')

    cropped_spect_img = crop_file_to_img(x_mid, y_mid, z_mid, spect_file)
    sitk.WriteImage(cropped_spect_img[:, :, :], 'crop_spect.nii.gz')

    cropped_mask_img = crop_file_to_img(x_mid, y_mid, z_mid, mask_file)
    sitk.WriteImage(cropped_mask_img[:, :, :], 'crop_mask.nii.gz')
