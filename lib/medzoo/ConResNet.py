...
class Conv3d(nn.Conv3d):
    ...
    def forward(self, x):
        ...
        print(f'weight_mean = {weight_mean.size()}')
        weight = weight - weight_mean
        std = torch.sqrt(torch.var(weight.view(weight.size(0), -1), dim=1) + 1e-12).view(-1, 1, 1, 1, 1)
        print(f'std = {std.size()}')
        weight = weight / std.expand_as(weight)
        print(f'after weight size = {weight.size()}')
...
class conresnet(nn.Module):
    ...
    def forward(self, x_list):
        x, x_res = x_list
        print(f'conresnet x_list = {len(x_list)}')
        print(f'conresnet x size = {x.size()}, x_res size = {x_res.size()}')

        ## encoder
        x = self.conv_4_32(x)
        print(f'x size after conv_4_32 = {x.size()}')
        x = self.layer0(x)
        skip1 = x
...
def CONRESNET(shape=(64, 64, 64), num_classes=1, weight_std=True):
