...
class ExtResNetBlock_Decoder(nn.Module):
    """
    Basic UNet block consisting of a SingleConv followed by the residual block.
    The SingleConv takes care of increasing/decreasing the number of channels and also ensures that the number
    of output channels is compatible with the residual block that follows.
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, order='cge', num_groups=8, scale_factor=(2, 2, 2),
                 decoder=True, **kwargs):
        super(ExtResNetBlock_Decoder, self).__init__()
    ...
    def forward(self, x):
        ...
        out = self.conv1(x)
        print(f'out after conv1 = {out.size()}')
        # residual = out

        # residual block
        out = self.conv2(out)
        print(f'out after conv2 = {out.size()}')
        out = self.upsampling(out)
        print(f'out after upsampling = {out.size()}')
        # add dropout
        # out = self.dropout3d(out)
        out = self.conv3(out)
        print(f'out after conv3 = {out.size()}')
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, scale_factor=(2, 2, 2), basic_module=DoubleConv,
        ...
        self.basic_module = ExtResNetBlock_Decoder(in_channels, out_channels,
                                         encoder=False,
                                         kernel_size=conv_kernel_size,
                                         order=conv_layer_order,
                                         num_groups=num_groups,
    ...
    def forward(self, encoder_features, x):
        # encoder_features = torch.Size([2, 512, 8, 8, 8]), x = torch.Size([2, 1024, 4, 4, 4])
        # print(f'encoder_features = {encoder_features.size()}, x = {x.size()}')
...
def create_decoders(f_maps, basic_module, conv_kernel_size, conv_padding, layer_order, num_groups, upsample):
    ...
    # reversed_f_maps = [1024, 512, 256, 128, 64]
    # print(f'reversed_f_maps = {reversed_f_maps}')
